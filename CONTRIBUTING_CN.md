# MindSpore Golden Stick 贡献指南

[View English](./CONTRIBUTING.md)

<!-- TOC -->

- [MindSpore Golden Stick贡献指南](#mindspore-golden-stick贡献指南)
    - [贡献者许可协议](#贡献者许可协议)
    - [快速入门](#快速入门)
    - [贡献流程](#贡献流程)
        - [代码风格](#代码风格)
...
请遵循此风格，以便MindSpore Golden Stick社区审查、维护和开发。
...
- 单元测试指南
    ...
    MindSpore Golden Stick社区使用Python单元测试框架[pytest](http://www.pytest.org/en/latest/)。注释名称需反映测试用例的设计意图。
...
- Fork MindSpore Golden Stick代码仓

    在提交代码至MindSpore Golden Stick项目之前，请确保已fork此项目到您自己的代码仓。MindSpore Golden Stick代码仓和您自己的代码仓之间可能会并行开发，请注意它们之间的一致性。
...
- 本地开发代码。
    ...
    以master分支为例，如果MindSpore Golden Stick需要创建版本分支和下游开发分支，请先修复上游的bug，再更改代码。
...
- 将代码推送到远程代码仓。
    ...
    git pull upstream master --rebase # 合并上游的变更。
    git push origin {新分支名称}
    ```

- 将请求拉取到MindSpore Golden Stick代码仓。

    在最后一步中，您需要在新分支和MindSpore Golden Stick主分支之间拉取比较请求。完成拉取请求后，Jenkins CI将自动设置，进行构建测试。拉取请求应该尽快合并到上游master分支中，以降低合并的风险。  

### 报告Issue

发现问题后，建议以报告issue的方式为MindSpore Golden Stick项目作出贡献。错误报告应尽量书写规范，内容详尽，感谢您对项目作出的贡献。

报告issue时，请参考以下格式：

- 说明您使用的环境版本（MindSpore Golden Stick、OS、Python等）。
- 说明是错误报告还是功能需求。
