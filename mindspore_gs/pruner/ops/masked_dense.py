...
# ============================================================================
"""Wrapper dense with mask."""

from mindspore import nn
from .masked_cell import MaskedCell


class MaskedDense(MaskedCell):
    """
    ...
    """
    def __init__(self, handler: nn.Dense, in_mask_shape=None, out_mask_shape=None):
        if not isinstance(handler, nn.Dense):
            raise TypeError(f'The parameter `handler` must be isinstance of nn.Dense, but got {type(handler)}.')
        if in_mask_shape is None:
            in_mask_shape = handler.in_channels
        if out_mask_shape is None:
            out_mask_shape = handler.out_channels
        super(MaskedDense, self).__init__(handler, in_mask_shape, out_mask_shape)
