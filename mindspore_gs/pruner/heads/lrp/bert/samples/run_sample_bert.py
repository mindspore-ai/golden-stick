...
from official.nlp.Bert.src import BertNetworkMatchBucket, \
    ...
    AdamWeightDecayForBert, AdamWeightDecayOp, BertPretrainEval
from official.nlp.Bert.src import BertModel
from official.nlp.Bert.src.utils import LossCallBack, BertLearningRate, EvalCallBack, BertMetric
...
def _get_optimizer(args_opt, network):
    ...
    if cfg.optimizer == 'Lamb':
        ...
        x_decay_params = list(filter(lambda x: 'log_a' not in x.name, decay_params))
        other_params = list(filter(lambda x: not cfg.AdamWeightDecay.decay_filter(x), params))
        x_other_params = list(filter(lambda x: 'log_a' not in x.name, other_params))
        group_params = [{'params': x_decay_params, 'weight_decay': cfg.AdamWeightDecay.weight_decay},
                        {'params': x_other_params, 'weight_decay': 0.0},
                        {'order_params': params},
        ...
        group_params = [{'params': x_decay_params, 'weight_decay': cfg.AdamWeightDecay.weight_decay},
                        ...
                        {'params': pruning_params, 'weight_decay': cfg.AdamWeightDecay.weight_decay, 'lr': 0.1},
                        {'order_params': params}]
        ...
        from official.nlp.Bert.src.utils import get_bert_thor_lr, get_bert_thor_damping
...
def init_net_with_grads(net_with_loss, optimizer):
    '''init net with grads'''
...
def run_pretrain():
    ...
    eval_ds = create_bert_dataset(device_num, rank, cfg.do_shuffle, cfg.eval_data_dir, cfg.schema_dir,
                                  cfg.batch_size,
                                  cfg.bucket_list, cfg.dataset_format, cfg.num_samples)

    bert_model = BertModel(bert_net_cfg, True)

    net_with_loss_hp = BertPreTrainingForGates(bert_net_cfg, bert_model)

    params_dict = {"config": bert_net_cfg,
                   "is_training": True,
                   "use_one_hot_embeddings": False}

    l0_penalty = 0.0015

    head_pruner_config = {"model_config": params_dict,
                          "l0_penalty": l0_penalty}

    factory_config = {"prune_type": PruningType.LRP,
                      "arch_type": SupportedModels.BERT,
                      "config": head_pruner_config}

    pruner = HeadPruningFactory.get_pruner(factory_config)

    net_with_loss_hp = pruner.apply(net_with_loss_hp)
    ...
    optimizer = _get_optimizer(cfg, net_with_loss_hp)
    ...
    if cfg.load_checkpoint_path:
        ...
        load_param_into_net(net_with_loss_hp, param_dict)

    net_with_grads = init_net_with_grads(net_with_loss_hp, optimizer)
    model = Model(net_with_grads)

    if cfg.train_with_eval == 'true':
        net_eval = BertPretrainEval(bert_net_cfg, network=net_with_loss_hp.bert)
    ...
    save_dir_path = ''
    prune_model = pruner.convert(model.train_network.network, save_dir_path)
    return prune_model
