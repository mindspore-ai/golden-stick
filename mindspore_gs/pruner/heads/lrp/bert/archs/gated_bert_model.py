...
class BertAttention(nn.Cell):
    ...
    def construct(self, from_tensor, to_tensor, attention_mask):
        ...
        # Calc the probabilities of gates
        reg = 0.0
        if self.has_gates:
        ...
        attention_probs = attention_probs.astype(mstype.float16)
...
class BertEncoderCell(nn.Cell):
    ...
    def construct(self, hidden_states, attention_mask):
        """Encoder cell construct"""
        # self-attention
        attention_output = self.attention(hidden_states, attention_mask)
        # feed construct
        intermediate_output = self.intermediate(attention_output[0])
        # add and normalize
        output = self.output(intermediate_output, attention_output[0])
    ...
    def construct(self, input_tensor, attention_mask):
        ...
        for layer_module in self.layers:
            ...
            if self.return_all_encoders:
                ...
                layer_output_reshape = self.reshape(layer_output[0], shape)
                all_encoder_layers = all_encoder_layers + (layer_output_reshape,)
...
class BertModel(nn.Cell):
    ...
    def construct(self, input_ids, token_type_ids, input_mask):
        ...
        sequence_output = self.cast(encoder_output[0][self.last_idx], self.dtype)

        total_reg = encoder_output[1]
        # pooler
        ...
        return sequence_output, pooled_output, embedding_tables, total_reg
