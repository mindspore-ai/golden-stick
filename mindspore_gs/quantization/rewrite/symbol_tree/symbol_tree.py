...
class SymbolTree(Observer, Observable, NodeManager):
    ...
    def _remove_duplicated_import(module_ast):
        ...
        class TransImportNode(ast.NodeTransformer):  # pylint: disable=invalid-name
            """Find all import nodes from input ast node."""

            def visit_ClassDef(self, node: ast.ClassDef) -> Any:  # pylint: disable=invalid-name
            ...
            def visit_Try(self, node: ast.Try) -> Any:  # pylint: disable=invalid-name
            ...
            def visit_Import(self, node: ast.Import) -> Any:  # pylint: disable=invalid-name
                import_str = astunparse.unparse(node)
                if import_str not in imports:
                    imports.add(import_str)
                    return node
            ...
            def visit_ImportFrom(self, node: ast.ImportFrom) -> Any:  # pylint: disable=invalid-name
                """
    ...
    def insert_to_ast_while_insert_function(new_node: CallFunction, base_node: Node, before_node: bool,
        ...
        from ..ast_helpers import AstFlattener  # pylint: disable=import-outside-toplevel
        ast_functiondef = AstFlattener().transform(ast_functiondef, [func_name], stree)
        # parse ast codes into CallFunction Node
        from ..parsers import ParserRegister  # pylint: disable=import-outside-toplevel
    ...
    def insert_to_ast_while_insert_node(new_node: Node, base_node: Node, before_node: bool):
        ...
        if not stree:
            raise ValueError("When inserting node to ast, the belonging symbol tree of new_node is None.")
    ...
    def append_python_node(self, ast_scope: ast.AST, ast_node: ast.AST, node_manager: NodeManager = None) -> Node:  # pylint: disable=arguments-differ
        """
    ...
    def erase_node(self, node_or_name: Union[Node, str]) -> Node:  # pylint: disable=arguments-renamed
        """
    ...
    def get_node_tabulate(self, all_nodes: bool = False) -> str:
        ...
        """
        try:
            from tabulate import tabulate # pylint: disable=unused-import,reportMissingModuleSource,import-outside-toplevel
        except ImportError:
    ...
    def dump(self):  # pylint: disable=arguments-differ
        """Dump graph."""
    ...
    def _cal_difference_set(self, input_set, other):  # pylint: disable=redefined-builtin
