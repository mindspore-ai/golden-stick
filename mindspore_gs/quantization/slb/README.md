...
A quantization network refers to a network with pseudo-quantized nodes generated by modifying the network layers requiring quantization based on the original network definition. By constructing the `SlbQuantAwareTraining` class under MindSpore Golden Stick and applying it to the original network, the original network is converted into a quantized network.
