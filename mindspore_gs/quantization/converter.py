...
# ============================================================================
"""convert QAT network to quantized network"""
import copy
import numpy as np
...
from .quant_utils import scale_zp_max_min_from_fake_quant_cell, fold_batchnorm, without_fold_batchnorm,\
    get_quant_min_max, weight2int

__all__ = ["Converter"]


class QuantAirBlock(Cell):
    r"""
    ...
    def __init__(self,
                 ...
                 bias=None,
                 activation=None):
        super(QuantAirBlock, self).__init__()
...
class Converter:
    """
    Convert quantization aware network to real deployed network. This function will convert network structure
    according to file format.

    Args:
        network (Cell): MindSpore quantization aware training network.
        mean (int, float): The mean of input data after preprocessing, used for quantizing the first layer of
            network. Default: 127.5.
        ...
        file_format (str): Export as .mindir or .air file format later, user can select "MINDIR" or "AIR". If select
            "MINDIR", converter will add quantization parameters to ops primitives and remove fake quantizers, if
            select "AIR", converter will insert quant and dequant ops between fp32 and quantized ops, and quantize
            weight parameters to int. Default: "MINDIR".

    Returns:
        Cell, Network to infer.
    """

    def __init__(self, network, mean=127.5, std_dev=127.5, file_format="MINDIR"):
    ...
    def convert(self):
        ...
        network = self._convert_quant2deploy(self.network)
        return network
    ...
    def __get_quant_param(self, cell_core, fake_quant_a_out, node):
        ...
        param_dict["weight_narrow_range"] = None
        param_dict["weight_narrow_range"] = cell_core.fake_quant_weight._narrow_range
        param_dict["mean"] = self.mean
        param_dict["std_dev"] = self.std_dev
        param_dict["symmetric"] = cell_core.fake_quant_weight._narrow_range
    ...
    def _get_quant_block(self, cell_core, activation, fake_quant_a_out, node):
        """convert network's quant subcell to deploy subcell"""
        scale_a_in, zp_a_in, scale_w, zp_w, param_dict = self.__get_quant_param(cell_core, fake_quant_a_out, node)
        ...
        if isinstance(activation, FakeQuantizer):
        ...
        if self.file_format == "MINDIR":
            ...
            block = QuantAirBlock(op_core, weight, quant_op, dequant_op, scale_deq, bias, activation)
        return block

    def _convert_subcell(self, network, change, name, subcell, node):
        """Convert subcell to ant subcell."""
        if subcell is not None and hasattr(subcell, "fake_quant_weight"):
            new_subcell = self._get_quant_block(subcell, None, None, node)
            prefix = subcell.param_prefix
            new_subcell.update_parameters_name(prefix + '.')
            self.upcell = new_subcell
            self.network_bk.insert_child_to_cell(name, new_subcell)
            change = True
        return network, change

    def _convert_quant_wrapped_cell(self, network, name, subcell: QuantizeWrapperCell, node):
        """Convert quant wrapped cell to ant subcell."""
        new_subcell = self._get_quant_block(subcell.get_handler(), None, subcell.get_output_quantizer(), node)
        prefix = subcell.param_prefix
        new_subcell.update_parameters_name(prefix + '.')
        self.upcell = new_subcell
        self.network_bk.insert_child_to_cell(name, new_subcell)
    ...
    def _convert_conv(self, network, change, name, subcell, node):
        ...
        if cell_core is not None and hasattr(cell_core, "fake_quant_weight"):
            new_subcell = self._get_quant_block(cell_core, activation, fake_quant_act, node)
            self.upcell = None
            prefix = subcell.param_prefix
            new_subcell.update_parameters_name(prefix + '.')
    ...
    def _convert_dense(self, network, change, name, subcell, node):
    ...
    def _convert_add(self, subcell):
        ...
        if isinstance(subcell.add, FakeQuantizer):
    ...
    def _convert_fake_quant_after_cell(self, network, name, subcell):
        ...
        if subcell.get_output_quantizer() is not None:
            self._add_output_min_max_for_op(subcell.get_handler(), subcell.get_output_quantizer())
            network.__delattr__(name)
            network.__setattr__(name, P.identity)

    def _convert_core_quant_subcell(self, network, change, name, subcell, node):
        """Convert subcell to ant subcell for conv and dense."""
        is_core_subcell = True
        if isinstance(subcell, nn.Conv2dBnAct):
            network, change = self._convert_conv(network, change, name, subcell, node)
        elif isinstance(subcell, nn.DenseBnAct):
            network, change = self._convert_dense(network, change, name, subcell, node)
            ...
            network, change = self._convert_subcell(network, change, name, subcell, node)
        elif isinstance(subcell, QuantizeWrapperCell):
            network, change = self._convert_quant_wrapped_cell(network, name, subcell, node)
            self.upcell = None
        else:
    ...
    def _convert_other_quant_subcell(self, network, change, name, subcell):
        ...
        elif isinstance(subcell, FakeQuantizer):
    ...
    def _convert_quant2deploy(self, network):
        ...
        change = False
        net_transform = NetTransformer(network)
        for node in net_transform.nodes():
            subcell = node.get_instance()
            cell_name = None
            for name in cells:
                if cells[name] == subcell:
                    cell_name = name
            if cell_name is not None:
                is_core_quant_subcell, network, change = self._convert_core_quant_subcell(network, change, cell_name,
                                                                                          subcell, node)
                is_other_quant_subcell, network, change = self._convert_other_quant_subcell(network, change, cell_name,
                                                                                            subcell)
