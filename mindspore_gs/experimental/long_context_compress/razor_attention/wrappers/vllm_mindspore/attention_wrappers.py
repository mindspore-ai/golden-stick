...
class RACompressCell(Cell):
    ...
    def _get_score(self, attn_score):
        ...
        for i in range(self.num_heads):
            ...
            logger.debug(
                f"RACompressCell: echo_score of Layer({self.layer_name}) head [{i}] "
                f"shape: {echo_score.shape}, dtype: {echo_score.dtype}"
            )
            logger.debug(
                f"RACompressCell: induction_score of Layer({self.layer_name}) head [{i}] "
                f"shape: {induction_score.shape}, dtype: {induction_score.dtype}"
            )
...
class DeployRACompressCell(Cell):
    ...
    def __init__(self, layer: Attention, cfg: RAConfig, retrieval_head: list):
        super().__init__()
        self.layer = layer
        self.cfg = cfg
    ...
    def _compress(self, cache, use_virtual_token=False):
        """Compress cache by keeping sink tokens and recent tokens.

        Args:
            cache: Input cache tensor to compress
            use_virtual_token: Whether to use virtual token compression

        Returns:
            Compressed cache tensor
        """
    ...
    def construct(
        ...
        if prefill_meta := attn_metadata.prefill_metadata:
            ...
            if not prefill_meta.prefill_metadata.chunked_prefill:  # type: ignore
                ...
                output = self.layer._run_prefill_forward(  # pylint: disable=protected-access
    ...
    def _run_decode_attention(
            ...
            seq_lens_arg, block_tables
    ):
        """Run decode attention with retrieval and non-retrieval heads.

        Args:
            query: Query tensor
            key_cache_retri: Key cache for retrieval heads
            value_cache_retri: Value cache for retrieval heads
            key_cache_non_retri: Key cache for non-retrieval heads
            value_cache_non_retri: Value cache for non-retrieval heads
            seq_lens_arg: Sequence lengths
            block_tables: Block tables for attention
