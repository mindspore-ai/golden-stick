...
from mindspore_gs.ptq.convert_utils import (
    convert_to_antiquant, convert_to_quant, convert_to_dequant, convert_to_dequant_bmm
)
from mindformers.modules import Linear
from mindformers.modules import KVCacheMgr
from mindspore_gs.ptq.quantizer_factory import QuantizerFactory
...
class PTQCell(QuantCell):
    ...
    @staticmethod
    def dequant_bmm_strategy(act_strategy,
                             weight_strategy,
                             is_transpose=False):
        ...
        ret_strategy = (act_strategy, weight_strategy, scale_strategy, offset_strategy, bias_strategy)
        return ret_strategy
...
class SQLinearWrapper(PTQCell, abc.ABC):
    ...
    def __init__(self, linear: Linear, policy=None, cfg=None):
        ...
        quant_factory = QuantizerFactory(cfg = self._cfg)
        self._input_quantizer = quant_factory.get_input_quantizer(**input_fq_args)
        self._weight_quantizer = quant_factory.get_weight_quantizer(self._linear.weight.name,
                                                                    weight_perchannel_args,
                                                                    **weight_fq_args)
        act_rank = 2
        self._act_observer = quant_factory.create_observer_perchannel(
            perchannel_args=PerChannelArgs(self._linear.in_channels, -1, act_rank),
            **input_fq_args
        )
        weight_observer_axis = 1 if linear.matmul.transpose_b else 0
        self._weight_in_observer = quant_factory.create_observer_perchannel(
            perchannel_args=PerChannelArgs(self._linear.in_channels, weight_observer_axis, rank),
            **weight_fq_args
        )
        self._output_quantizer = None
        prex = ""
        ...
        self._input_scale = Parameter(
            Tensor([1.0] * self._linear.in_channels), name=f'{prex}_input_scale'
        )
        self._scale_store = Parameter(
            Tensor([1.0] * self._linear.in_channels), name=f'{prex}_scale_store'
        )
        self._is_deploy = self._cfg.algo_args.get('is_deploy', None)
        if self._is_deploy is None:
            raise ValueError(f'Smooth quant shall have param is_deploy in algo_args, bug got None')
    ...
    def _create_scale_param(self, rank, name):
        ...
        if rank == bmm_rank:
            param = Parameter(
                Tensor([[[1.0] * self._linear.in_channels]], dtype=dtype.float32), name=name
            )
        else:
            param = Parameter(
                Tensor([[1.0] * self._linear.in_channels], dtype=dtype.float32), name = name
    ...
    def _calc_input_scale(self):
        input_max_pow = self._act_pow(self._act_observer.float_max, self._alpha)
        weight_max_pow = self._weight_pow(self._weight_in_observer.float_max, 1 - self._alpha)
    ...
    def convert(self, backend: Backend = Backend.GE_ASCEND):
        ...
        if backend == Backend.GE_ASCEND:
            ...
            self._output_quantizer = convert_to_dequant_bmm(self._input_quantizer,
                                                            ...
                                                            strategy=self.dequant_bmm_strategy(self._act_strategy,
                                                                                               self._weight_strategy,
                                                                                               is_transpose=self._linear.transpose_b))
            self._input_quantizer = convert_to_quant(self._input_quantizer,
                                                     strategy=(self._act_strategy,))
