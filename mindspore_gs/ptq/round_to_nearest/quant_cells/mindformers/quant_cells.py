...
class TeleLinearQuant(PTQCell):
    ...
    def __init__(self, linear: TelechatLinear, policy: PTQLayerPolicy):
        super(TeleLinearQuant, self).__init__(linear, policy)
        self._linear = linear
        rank = len(linear.weight.shape)
        self._weight_axis = rank - 2 if linear.matmul.transpose_b else rank - 1
        input_fq_args = {}
    ...
    def construct(self, x):
        ...
        if self._linear.expert_flag:
            if self._linear.use_expert_group_size is True:
                x = self.reshape(x, (-1, self._linear.expert_num, self._linear.expert_group_size,
                                    self._linear.in_channels))
            else:
                x = self.reshape(x, (self._linear.outer_batch, self._linear.expert_num, -1, self._linear.in_channels))
        ori_dtype = F.dtype(x)
