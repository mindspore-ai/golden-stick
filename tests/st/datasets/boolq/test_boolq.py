...
def check_ds(ds_path: str, bs: int, seq_length: int, vocab_file: str, repeat):
    ...
    ds = create_boolq_dataset(ds_path, "eval", bs, seq_length, tokenizer, repeat=repeat, need_pad=True)
    samples = 3270
    assert ds.get_repeat_count() == repeat
    assert ds.output_types()[0] == np.int32
...
def test_boolq(device):
    ...
    boolq_ds = os. path.join(cur_dir, "/home/workspace/mindspore_dataset/boolq/dev.jsonl")
