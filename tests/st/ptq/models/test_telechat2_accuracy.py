...
if __name__ == "__main__":
    ...
    q_ckpt_path = os.path.join(cur_dir, f"telechat2-quant-4p-{input_quant_algo}")
...
def ptq_predict_2stage_4p_run(quant_algo):
    ...
    return_code = os.system(
        ...
        f"--master_port={port} --join=True --log_dir=./test_ptq_{quant_algo}_predict_telechat2_4p_logs "
        f"python {run_file} -a {quant_algo}"
    )
    os.system("ps -u | grep 'test_telechat2_accuracy' | grep -v grep | awk -F ' ' '{print$2}' | xargs kill -9")
    os.system(f"kill -9 $(lsof -i:{port} | " + "awk '{print $2}')")
    time.sleep(1.0)
