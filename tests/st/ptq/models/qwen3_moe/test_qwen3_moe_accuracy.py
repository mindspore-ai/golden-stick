...
def ptq_predict_2stage_4p_run(quant_algo):
    ...
    run_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ptq_qwen3_moe_runner.py")
    ...
    if return_code != 0:
        log_file = open(f"./test_ptq_{quant_algo}_predict_qwen3_moe_4p_logs/worker_0.log", "r", encoding="utf-8")
        for line in log_file:
            print(line, flush=True)
        log_file.close()
    os.system("ps -u | grep 'ptq_qwen3_moe_runner' | grep -v grep | awk -F ' ' '{print$2}' | xargs kill -9")
    os.system(f"kill -9 $(lsof -i:{port} | " + "awk '{print $2}')")
    try:
        log_file = f"./test_ptq_{quant_algo}_predict_qwen3_moe_4p_logs/worker_0.log"
        print(f"to rm dir: {log_file}", flush=True)
        shutil.rmtree(log_file)
    except (OSError, FileNotFoundError):
        pass
    try:
        log_dir = f"./test_ptq_{quant_algo}_predict_qwen3_moe_4p_logs"
