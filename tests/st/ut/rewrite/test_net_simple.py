...
from mindspore_gs.quantization.rewrite import SymbolTree, ScopedValue, ValueType, Node
...
class SimpleNet(nn.Cell):
    ...
    def construct(self, x):
        """Method implementation."""
        x = self.conv1(x)
        x = x  # noqa: PLW0127  # pylint: disable=self-assigning-variable
        y = self.var
        y = y * 5
        y = y and True
        x = self.relu(x)
