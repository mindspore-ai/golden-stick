...
from mindspore_gs.quantization.rewrite import SymbolTree, ScopedValue, Node, NodeType
...
def add_relu_in_conv11(stree: SymbolTree):
    ...
    for node in stree.nodes():
        ...
        if node.get_name() == "conv11":
            conv11_stree: SymbolTree = node.get_sub_tree()
            for inner_node in conv11_stree.nodes():
                if inner_node.get_node_type() != NodeType.Output:
                    continue
                position = conv11_stree.before(inner_node)
                new_relu = nn.ReLU()
                new_relu_node = Node.create_call_cell(new_relu,
                                                      targets=[stree.unique_name('x')],
                                                      name='relu1',
                                                      args=[ScopedValue.create_naming_value('x')])
                conv11_stree.insert(position, new_relu_node)
                conv11_stree.set_output(0, new_relu_node.get_targets()[0].value)
                break
            break
