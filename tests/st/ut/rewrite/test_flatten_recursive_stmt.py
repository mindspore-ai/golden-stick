...
# ============================================================================
"""Test for AstFlattener in recursive statement handling."""
import ast
import inspect
...
class Network(Cell):
    """Network for testing AstFlattener."""
    def __init__(self):
        super().__init__()
...
def test_flatten():
    ...
    if ast_class.body and isinstance(ast_class.body[0], ast.Expr) and isinstance(ast_class.body[0].value, ast.Constant):
        body_idx = 1
    # Should have __init__ and construct methods (plus docstring if present)
    assert len(ast_class.body) == 2 + body_idx
