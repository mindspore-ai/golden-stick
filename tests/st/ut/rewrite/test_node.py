...
from mindspore_gs.quantization.rewrite import ScopedValue
from mindspore_gs.quantization.rewrite import SymbolTree as SymbolTreeApi
from mindspore_gs.quantization.rewrite import Node as NodeApi
from mindspore_gs.quantization.rewrite.node import Node


class FakeCell(Cell):
    """FakeCell class for testing."""
...
def test_create_by_cell():
    ...
    assert node._args_num == 2  # pylint: disable=protected-access
    assert node._kwargs_num == 1  # pylint: disable=protected-access
    assert node._normalized_args_keys == ["input1", "input2", "cool_boy"]  # pylint: disable=protected-access

    assert node._normalized_args == {  # pylint: disable=protected-access
