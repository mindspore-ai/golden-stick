...
# ============================================================================
"""Test Cellcontainer tests."""

from mindspore import nn
from mindspore.ops import operations as P

from mindspore_gs.quantization.rewrite import SymbolTree, NodeType, Node, ScopedValue, PatternEngine, Replacement, \
    PatternNode
...
class ResidualBlock(nn.Cell):
    """ResidualBlock class for testing."""
    expansion = 4
...
class ResNetSimple(nn.Cell):
    """ResNetSimple class for testing."""
    def __init__(self):
        """Method implementation."""
...
def test_cellcontainer_insert_ok():
    ...
    assert hasattr(cell_container._cells["0"], "new_conv")  # pylint: disable=protected-access
...
def test_cellcontainer_del():
    ...
    net = ResNetSimple()
    stree = SymbolTree.create(net)
    original_nodes_size = len(stree.get_handler()._nodes)  # pylint: disable=protected-access
    for node in stree.nodes():
        if node.get_node_type() == NodeType.CellContainer and node.get_name() == "layer1":
            ...
            for user in users:
                user.set_arg(0, "x")
            stree.erase(node)
    assert len(stree.get_handler()._nodes) == original_nodes_size - 1  # pylint: disable=protected-access
...
def test_cellcontainer_del_node_in_subtree():
    ...
    def _del_node(sub_tree):
        """Method for testing."""
        for del_node in sub_tree.nodes():
            if del_node.get_name() == "conv2":
                users = Node(del_node).get_users()
                for user in users:
                    user.set_arg(0, "out")
                sub_tree.erase_node(del_node)
    net = ResNetSimple()
    stree = SymbolTree.create(net)
    for node in stree.nodes():
        if node.get_node_type() == NodeType.CellContainer:
            for i, n in enumerate(node.get_handler().node_list):
                if n.get_node_type() == NodeType.Tree and i == 1:
                    ...
                    original_nodes_size = len(sub_tree._nodes)  # pylint: disable=protected-access
                    _del_node(sub_tree)
                    assert len(sub_tree._nodes) == original_nodes_size - 1  # pylint: disable=protected-access
...
def test_cellcontainer_replace_node():
    ...
    for node in stree.nodes():
        if node.get_node_type() == NodeType.CellContainer and node.get_name() == "layer1":
            ...
            break
    new_net = stree.get_network()
    assert isinstance(new_net.layer1._cells["0"], nn.Conv2d)  # pylint: disable=protected-access
...
def test_cellcontainer_pattern():
    ...
    class ConvBnReplacement(Replacement):
        """Replacement for Conv-BN pattern."""
    ...
    net = ResNetSimple()
    stree = SymbolTree.create(net)
    pattern_obj = ConvReluPattern()
    pattern_obj.apply(stree)
    ...
    assert hasattr(cell_container._cells["0"], "new_relu")  # pylint: disable=protected-access
    assert hasattr(cell_container._cells["0"], "new_maxpool1")  # pylint: disable=protected-access
    assert isinstance(getattr(getattr(cell_container._cells["0"], "down_sample_layer"), "0"), nn.MaxPool2d)  # pylint: disable=protected-access
    assert hasattr(cell_container._cells["1"], "new_relu")  # pylint: disable=protected-access
    assert hasattr(cell_container._cells["1"], "new_maxpool1")  # pylint: disable=protected-access
    assert isinstance(getattr(getattr(cell_container._cells["1"], "down_sample_layer"), "0"), nn.MaxPool2d)  # pylint: disable=protected-access
    assert hasattr(cell_container._cells["2"], "new_relu")  # pylint: disable=protected-access
    assert hasattr(cell_container._cells["2"], "new_maxpool1")  # pylint: disable=protected-access
    assert isinstance(getattr(getattr(cell_container._cells["2"], "down_sample_layer"), "0"), nn.MaxPool2d)  # pylint: disable=protected-access
