...
class DeepseekInferParallelism(BaseModelParallelism):
    ...
    def smooth_quant_routed_experts_gate_hidden_split(self, param_name, src_hf_dir, hf_weight_map):
        '''smooth_quant_routed_experts_gate_hidden_split'''
    ...
    def smooth_quant_routed_experts_w2_split(self, param_name, src_hf_dir, hf_weight_map):
        '''smooth_quant_routed_experts_w2_split'''
        if param_name.endswith(".weight"):
            value, _ = self.get_safetensor_from_file(param_name, src_hf_dir, hf_weight_map)
            value_list = []
            for experts_id in range(value.shape[0]):
    ...
    def smooth_quant_row_linear_split(self, param_name, src_hf_dir, hf_weight_map):
        '''smooth_quant_row_linear_split'''
        if param_name.endswith(".weight"):
    ...
    def infer_smooth_quant_net_ms_convert_layer_weight(self, src_hf_dir, num_layers, hf_weight_map):
        ...
        for param_name, _ in tqdm(hf_weight_map.items(), desc="split safetensors"):
            ...
            if any([name in param_name for name in no_need_split_layer]):
                ...
                value = self.smooth_quant_row_linear_split(param_name, src_hf_dir, hf_weight_map)
            elif ".routed_experts.ffn.w_gate_hidden." in param_name:
                value = self.smooth_quant_routed_experts_gate_hidden_split(param_name, src_hf_dir, hf_weight_map)
            elif ".routed_experts.ffn.w2" in param_name:
                value = self.smooth_quant_routed_experts_w2_split(param_name, src_hf_dir, hf_weight_map)
            elif any([name in param_name for name in ["lkv2kv_k_nope", "lkv2kv_v"]]):
                value, _ = self.get_safetensor_from_file(param_name, src_hf_dir, hf_weight_map,
                                                         is_split_param=True, split_axis=0)
            elif "lm_head" in param_name:
    ...
    def infer_convert_and_parallelism(self, src_hf_dir):
        ...
        if not quant_method or quant_method not in ('gptq-pergroup', 'smoothquant'):
