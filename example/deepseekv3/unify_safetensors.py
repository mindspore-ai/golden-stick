...
def _process_safetensors_file(folder_path, ckpt_path, rank_num, ffn_split, qkv_split):
...
def _process_name_map_json(param_json_path, ckpt_path, ffn_split, qkv_split):
    """Process a name_map.json file."""
    # Normalize and validate the JSON file path
    param_json_path = os.path.realpath(param_json_path)
    if not param_json_path.startswith(ckpt_path):
...
def split_for_smooth_quant(ckpt_path, rank_num, ffn_split, qkv_split):
    ...
    for folder_name in os.listdir(ckpt_path):
        if folder_name.endswith(".safetensors"):
            folder_path = os.path.join(ckpt_path, folder_name)
            _process_safetensors_file(folder_path, ckpt_path, rank_num, ffn_split, qkv_split)
        elif folder_name.endswith('name_map.json'):
            param_json_path = os.path.join(ckpt_path, folder_name)
            _process_name_map_json(param_json_path, ckpt_path, ffn_split, qkv_split)
