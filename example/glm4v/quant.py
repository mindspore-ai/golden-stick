...
def get_args():
    ...
    parser = argparse.ArgumentParser(description="Quantization of GLM4V series models.")
    ...
    parser.add_argument('--quant_type',
                        ...
                        help="Quantization type, available: a8w8_smooth_quant/"
                             "a8w8_osl/a16w8/a8dyw8/a16w4_awq/a16w4_gptq")
...
def create_ptq_config(quant_type):
    ...
    if quant_type.lower() == "a8w8_smooth_quant":
        ...
        cfg = PTQConfig(weight_quant_dtype=msdtype.int8,
                        opname_blacklist=['lm_head', 'visual', 'down_proj'])
        layer_policies = OrderedDict()
    elif quant_type.lower() == "a8dyw8":
        cfg = PTQConfig(weight_quant_dtype=msdtype.int8,
                        act_quant_dtype=msdtype.int8,
                        act_quant_granularity=QuantGranularity.PER_TOKEN,
                        opname_blacklist=['lm_head.', 'visual', 'down_proj'])
        layer_policies = OrderedDict()
    elif quant_type.lower() == "a16w4_awq":
        cfg = PTQConfig(weight_quant_dtype=msdtype.qint4x2,
                        ...
                        opname_blacklist=['lm_head', 'visual', 'down_proj'])
        layer_policies = OrderedDict()
    elif quant_type.lower() == "a16w4_gptq":
        cfg = PTQConfig(weight_quant_dtype=msdtype.qint4x2,
